...
**Budget ↔ Milestone reconciliation** _(added 2026-05-07)_. For any proposal whose **## Budget & Resources** section is not "Not applicable.", the total funding amount declared in **## Budget & Resources** must equal the sum of the per-milestone allocations shown in the **## Milestones** table. If the Milestones table does not include a per-milestone budget column, the proposal fails this rule unless **## Implementation Plan** contains an explicit paragraph explaining why a milestone-by-milestone breakdown is not feasible (e.g., the funding is a single up-front allocation tied to a single deliverable). On failure, the suggestedEdit should add a **Budget** column to the Milestones table reusing the proposal's existing milestone names and adding placeholder amounts that sum to the declared total.
